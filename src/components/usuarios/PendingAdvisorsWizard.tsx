...
import { ChevronLeft, ChevronRight, Save, SkipForward, Trash2, Eye, EyeOff, Loader2, AlertTriangle } from 'lucide-react';
...
export default function PendingAdvisorsWizard({ open, onOpenChange, onComplete }: PendingAdvisorsWizardProps) {
  const { user } = useAuth();
  const { pendingList, markAsCreated, markAsDiscarded, updatePendingAdvisor } = usePendingAdvisors();
  ...
  const regionalName = regionales.find(r => r.id === advisor.regional_id)?.nombre || 
    regionales.find(r => r.codigo === advisor.cod_region)?.nombre || 'Sin regional';
  
  const filteredJefes = jefesVentas.filter(j => !advisor.regional_id || j.regional_id === advisor.regional_id);

  const canSave = form.email && form.password && form.password.length >= 6 && form.tipo_asesor;
    ...
    try {
      ...
      const response = await dataService.functions.invoke<{ error?: string; user?: { id: string } }>('create-user', {
        body: {
          ...
          zona: advisor.regional_id ? getZonaByRegional(regionalName) || undefined : undefined,
            ...
            <div className="p-3 bg-muted rounded-lg space-y-2">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium">{regionalName}</p>
                </div>
                <div>
            ...
            <div className="space-y-3">
              <div className="grid gap-2">
                ...
                <Input
                  id="wiz-email"
                  type="email"
                  placeholder="asesor@electrocreditos.com"
                  value={form.email}
